...
from templisafe.util import dict_to_frozenset
...
class SchemaParserSettings(Settings, ABC):
    ...
    schema_key: str = Field(
        "schema", 
        description="The top-level key in the schema configuration"
        )
    type_key: str = Field(
        "type", 
        description="The key for variable types in the schema configuration"
        )
    ...
    allowed_types: tuple[str, ...] = Field(
        ...
        description="The variable types allowed"
        )
