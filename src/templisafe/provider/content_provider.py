...
from templisafe.executor.source_executor import SourceExecutor, SourceExecutorRequest, SourceExecutorResult, SourceRequest
...
class ContentProvider:
    ...
    def __init__(self, source_executor_resolver: SourceExecutorResolver, strategy_optimizer: StrategyOptimizer) -> None:
    ...
    def _create_request(self, source_group: SourceGroup) -> SourceExecutorRequest:
        return SourceExecutorRequest(
            [
                SourceRequest(id=id, source=s) for id, s in source_group.sources.items()
            ]
        )
    
    def _create_content_group(self, result: SourceExecutorResult) -> ContentGroup:
        return ContentGroup(
            {r.id: r.content for r in result.results}
        )
