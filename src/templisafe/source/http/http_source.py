...
class HttpSessionPool:
    ...
    async_pool: HttpAsyncSessionPool | None = None
...
class HttpSource(AsyncSource):
    ...
    def _get_async_pool(self) -> HttpAsyncSessionPool:
        if self._session_pool.async_pool is None:
            ...
            self._session_pool.async_pool = factory._create_async_pool(
                async_manager,
                self.settings.async_session_settings,
            )
        return self._session_pool.async_pool
    ...
    async def aopen(self) -> None:
        self._async_session = await self._get_async_pool().acquire()

    @overrides
    async def aclose(self) -> None:
        if self._async_session is not None:
            async_pool = self._session_pool.async_pool
            if async_pool is not None:
                await async_pool.release(self._async_session)
            self._async_session = None

    @overrides
    async def aread(self) -> str:
        session: Any | None = self._async_session
        if session is None:
            raise UninitializedSourceError()
