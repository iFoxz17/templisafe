...
def sources_inline() -> list[Source]:
    ...
    return [
        InlineSource(settings=InlineSourceSettings(content_type=ContentType.TEXT, content="Hello")),
        InlineSource(settings=InlineSourceSettings(content_type=ContentType.TEXT, content="World")),
    ]
...
def sources_mixed() -> list[Source]:
    ...
    return [
        InlineSource(settings=InlineSourceSettings(content_type=ContentType.TEXT, content="A")),
        LocalSource(settings=LocalSourceSettings(content_type=ContentType.TEXT, path="/tmp/file.txt")),
        HttpSource(settings=HttpSourceSettings(content_type=ContentType.TEXT, url="http://example.com")),
    ]
...
def test_source_weights_explicit(optimizer: StrategyOptimizer):
    ...
    assert (
        optimizer._source_weight(LocalSource(LocalSourceSettings(content_type=ContentType.TEXT, path="/tmp/x"))) == 10
    )
